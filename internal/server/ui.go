...
async function load(){var r=await fetch(A+'/releases').then(function(r){return r.json()});items=r.releases||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.version||'').toLowerCase().includes(q)||(i.changelog||'').toLowerCase().includes(q)||(i.status||'').toLowerCase().includes(q)});
...
if(i.version)h+='<div class="item-sub">'+esc(i.version)+'</div>';
...
if(i.asset_count)h+='<span>'+i.asset_count+'</span>';
if(i.downloads)h+='<span>'+i.downloads+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.published_at)h+='<span>'+esc(i.published_at)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';
if(i.changelog)h+='<div class="item-notes">'+esc((i.changelog||'').substring(0,200))+'</div>';
...
function formHTML(item){var i=item||{name:"",version:"",changelog:"",asset_count:0,downloads:0,status:"",published_at:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' ORDNANCE</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Version</label><input id="f-version" value="'+esc(i.version)+'"></div>';
h+='<div class="fr"><label>Changelog</label><textarea id="f-changelog" rows="3">'+esc(i.changelog)+'</textarea></div>';
h+='<div class="fr"><label>Asset Count</label><input id="f-asset-count" type="number" value="'+(i.asset_count||0)+'"></div>';
h+='<div class="fr"><label>Downloads</label><input id="f-downloads" type="number" value="'+(i.downloads||0)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>Published At</label><input id="f-published-at" type="date" value="'+esc(i.published_at)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),version:document.getElementById('f-version').value.trim(),changelog:document.getElementById('f-changelog').value.trim(),asset_count:parseInt(document.getElementById('f-asset-count').value)||0,downloads:parseInt(document.getElementById('f-downloads').value)||0,status:document.getElementById('f-status').value.trim(),published_at:document.getElementById('f-published-at').value.trim()};
if(editId){await fetch(A+'/releases/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/releases',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
