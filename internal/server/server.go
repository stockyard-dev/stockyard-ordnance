package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-ordnance/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-ordnance"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/scans",s.handleList)
    s.mux.HandleFunc("POST /api/scans",s.handleCreate)
    s.mux.HandleFunc("DELETE /api/scans/{id}",s.handleDelete)
    s.mux.HandleFunc("POST /api/scans/{id}/vulns",s.handleAddVuln)
    s.mux.HandleFunc("GET /api/scans/{id}/vulns",s.handleGetVulns)
